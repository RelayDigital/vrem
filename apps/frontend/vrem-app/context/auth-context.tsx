...
export function AuthProvider({ children }: { children: ReactNode }) {
        ...
        const initAuth = async () => {
            ...
            if (storedToken) {
                ...
                try {
                    ...
                    const resolvedOrgId = storedOrg || orgs[0]?.orgId || user.organizationId || null;
                    if (resolvedOrgId) {
    ...
    const login = async (credentials: { email: string; password: string }) => {
        ...
        try {
            ...
            const orgs = await api.organizations.listMine();
            setMemberships(orgs);
            const resolvedOrgId = api.organizations.getActiveOrganization() || orgs[0]?.orgId || response.user.organizationId || null;
            if (resolvedOrgId) {
                api.organizations.setActiveOrganization(resolvedOrgId);
            }
            setActiveOrganizationId(resolvedOrgId);
    ...
    return (
        <AuthContext.Provider value={{ user, memberships, activeOrganizationId, token, isLoading, login, register, logout, switchOrganization }}>
            {children}
