...
export function PhotographerManagement({ photographers }: PhotographerManagementProps) {
  const activePhotographers = photographers.filter((p) => p.status === 'active');
  const avgRating =
    photographers.reduce((sum, p) => sum + p.rating.overall, 0) / photographers.length;
  const avgOnTimeRate =
    photographers.reduce((sum, p) => sum + p.reliability.onTimeRate, 0) / photographers.length;
