...
export function MapView({
    ...
    const checkDimensions = () => {
      if (container.offsetWidth === 0 || container.offsetHeight === 0) {
        ...
        if (retryCount >= maxRetries) {
          console.error(
            "Map container never got dimensions, initializing anyway"
          );
  ...
  const getStatusColor = (status: string): string => {
    ...
    return (
      statusMap[status] ||
      getCSSVar(`--status-${statusVarName}`) ||
      getCSSVar("--status-cancelled") ||
      "#9ca3af"
    );
  };
  ...
  const getPriorityColor = (priority: string): string => {
    ...
    return (
      priorityMap[priority] || getCSSVar("--priority-standard") || "#6b7280"
    );
  };
  ...
  const createIconElement = (
    iconComponent: typeof Home | typeof User,
    color: string,
    size: number = 32
  ): HTMLElement => {
  ...
  useEffect(() => {
    ...
    const otherJobs = jobs.filter(
      (j) => !["pending", "assigned", "in_progress"].includes(j.status)
    );

    // Helper to create popup content
    const createJobPopupContent = (
      job: JobRequest,
      statusLabel: string,
      statusBg: string,
      statusColor: string
      ...
      const priorityBgMap: Record<string, string> = {
        urgent: priorityColorValue
          ? addOpacity(priorityColorValue, 0.2)
          : "#fee2e2",
        rush: priorityColorValue
          ? addOpacity(priorityColorValue, 0.2)
          : "#fef3c7",
        standard: getMutedBackgroundColor(),
      };
      const priorityBg =
        priorityBgMap[job.priority] || getMutedBackgroundColor();

      // Only show images for delivered jobs (when technician has completed the job)
      const hasImage = !!(
        job.propertyImage &&
        job.propertyImage.trim() !== "" &&
          ...
          <div style="padding: 10px; flex: 1; display: flex; flex-direction: column; justify-content: center;">
            ...
            ${
              job.estimatedDuration
                ? `
              <div style="font-size: 9px; color: ${getMutedTextColor()}; margin-top: 5px; padding-top: 5px; border-top: 1px solid ${
                    getCSSVar("--border") || "#e5e7eb"
                  };">
    ...
    pendingJobs.forEach((job) => {
      ...
      const pendingBg = pendingColorValue
        ? addOpacity(pendingColorValue, 0.2)
        : "#fee2e2";
      const pendingText =
        getCSSVar("--status-pending-foreground") || getTextColor();
    ...
    assignedJobs.forEach((job) => {
      ...
      const assignedColor = getStatusColor(
        job.status === "assigned" ? "assigned" : "in_progress"
      );
      ...
      const assignedTechnician = assignedId
        ? effectiveTechnicians.find((p) => p.id === assignedId)
        : undefined;
      const statusLabel =
      ...
      const assignedBg = assignedColorValue
        ? addOpacity(assignedColorValue, 0.2)
        : "#dbeafe";
      ...
      const popupContent = createJobPopupContent(
        job,
        statusLabel,
        assignedBg,
        assignedText
    ...
    otherJobs.forEach((job) => {
      ...
      const getStatusConfig = (status: string) => {
        ...
        const statusBg = statusColorValue
          ? addOpacity(statusColorValue, 0.2)
          : getMutedBackgroundColor();
        const statusText =
          getCSSVar(`--status-${statusVarName}-foreground`) || getTextColor();
        return { bg: statusBg, color: statusText };
      };

      const statusConfig = getStatusConfig(job.status) || {
        bg: getMutedBackgroundColor(),
        color: getTextColor(),
      };
    ...
    effectiveTechnicians.forEach((technician) => {
      ...
      const deliveredBg = deliveredColorValue
        ? addOpacity(deliveredColorValue, 0.2)
        : "#d1fae5";
      // Use the status color itself for text (not foreground) to ensure good contrast on light background
      ...
      const cancelledBg = cancelledColorValue
        ? addOpacity(cancelledColorValue, 0.2)
        : getMutedBackgroundColor();
      // Use the status color itself for text (not foreground) to ensure good contrast on light background
      const cancelledText = cancelledColor || getMutedTextColor();
      const rushColorValue = getCSSVar("--priority-rush");
      const rushBg = rushColorValue
        ? addOpacity(rushColorValue, 0.2)
        : "#fef3c7";
      ...
      const popupContent = `
            ...
            <div style="width: 64px; min-width: 64px; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; padding-top: 12px; background: ${getMutedBackgroundColor()};">
              ...
              `
                ...
                <div style="width: 40px; height: 40px; border-radius: 50%; background: ${
                  isAvailable ? deliveredBg : cancelledBg
                }; display: flex; align-items: center; justify-content: center; border: 2px solid ${availableColor}; color: ${
                      isAvailable ? deliveredText : cancelledText
        ...
        isAvailable ? deliveredText : cancelledText
      }; border-radius: 3px; font-size: 8px; font-weight: 500;">
            ...
            <div style="padding: 10px; flex: 1; display: flex; flex-direction: column; justify-content: center;">
              ...
              <div style="font-size: 10px; color: ${getMutedTextColor()}; margin-bottom: 3px;">
                <span style="font-weight: 500;">Location:</span> ${getLocationDisplay(
              ...
              <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 3px; margin-top: 6px; padding-top: 6px; border-top: 1px solid ${
                getCSSVar("--border") || "#e5e7eb"
              };">
                <div style="text-align: center;">
                  <div style="font-size: 12px; font-weight: 600; color: ${getTextColor()};">${
        technician.reliability.totalJobs
                ...
                <div style="text-align: center;">
                  <div style="font-size: 12px; font-weight: 600; color: ${getTextColor()};">${(
        technician.reliability.onTimeRate * 100
      ).toFixed(0)}%</div>
        ...
        technician.rating.overall
              ...
              ${
                technician.bio
                  ? `
                <div style="font-size: 9px; color: ${getMutedTextColor()}; margin-top: 6px; padding-top: 6px; border-top: 1px solid ${
                      getCSSVar("--border") || "#e5e7eb"
  ...
  useEffect(() => {
        ...
        if (data.code === "Ok" && data.routes && data.routes[0]) {
          ...
          try {
            ...
            const animateGradient = () => {
              ...
              const sortedStops = Array.from(uniqueStops.entries()).sort(
                (a, b) => a[0] - b[0]
              );

              // Construct the expression
              const gradientExpression: any[] = [
                "interpolate",
                ["linear"],
                ["line-progress"],
              ];
    ...
    mapRef.current,
    isLoaded,
    effectiveSelectedTechnician?.id,
    selectedJob?.id,
